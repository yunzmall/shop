...
<div>
    ...
    <a class="btn btn-success btn-sm disbut" href="javascript:;" onclick="send(this)" data-url="order.operation.send"  data-toggle="modal"
       data-target="#modal-confirmsend">确认发货</a>
</div>
@endif
{{--多包裹发货--}}
@if ($order['status'] == 2 && $order['is_all_send_goods'] == 1)
    <div>
        <input class='addressdata' type='hidden' value='{!! json_encode($order['address']) !!}' />
        <input class='itemid' type='hidden' value="{{$order['id']}}"/>
        <a class="btn btn-success btn-sm disbut" href="javascript:;" onclick="send(this)"  data-url="order.operation.add-order-express"  data-toggle="modal"
           data-target="#modal-confirmsend">继续发货</a>
    </div>
@endif
