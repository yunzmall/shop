                ...
                <el-form :inline="true" :model="search_form" class="demo-form-inline">
                    ...
                    <el-form-item label="">
                        ...
                        <el-select v-model="search_form.first_order" clearable placeholder="是否搜索首单" style="width:150px">
                            <el-option label="不搜索首单" value="0"></el-option>
                ...
                <div v-for="(item,index) in list" style="border:1px solid #e9e9e9;border-radius:10px;margin-bottom:10px">
                    <div class="list-info">
                        <div style="display:flex;flex-wrap:wrap">
                            ...
                            {{--自提点--}}
                            <div v-if="item.package_deliver" class="vue-ellipsis" style="color:#29BA9C;max-width:230px">
                                <strong>[[item.package_deliver.package_deliver_name]]</strong>&nbsp;&nbsp;&nbsp;
                            </div>
                            <div v-if="item.package_deliver && item.package_deliver.deliver_clerk_name  " class="vue-ellipsis" style="color:#999;max-width:230px">
                                <strong>[[item.package_deliver.deliver_clerk_name]]</strong>&nbsp;&nbsp;&nbsp;
                            </div>
                        ...
                        <div style="flex:1;text-align:right;min-width:150px;">
                            <a @click="closeOrder(item.id)" style="color:#29BA9C;font-size:13px;font-weight:600" v-if="item.status == 0">关闭订单</a>
                            <a @click="closeOrder1(item.id)" style="color:#29BA9C;font-size:13px;font-weight:600" v-if="item.status > 0 && item.canRefund ">退款并关闭订单</a>
                        </div>
                    </div>
                    <div class="list-con">
                        <div style="flex:3;min-width:400px">
                            <div v-for="(item1,index1) in item.has_many_order_goods" class="list-con-goods">
                                ...
                                <div class="list-con-goods-text" :style="{justifyContent:(item1.goods_option_title?'':'center')}">
                                    <div class="list-con-goods-title" style="color:#29BA9C;cursor: pointer;" @click="gotoGoods(item1.goods_id)">[[item1.title]]</div>
...
<script>
    var app = new Vue({
        ...
        methods: {
            ...
            //不需要再去单独请求，直接从页面返回
            commonPart() {
                this.$http.post(this.commonPartUrl).then(function(response) {
                    if (response.data.result) {
                        this.extra_param = response.data.data.extraParam;
            ...
            sureCloseOrder() {
                ...
                this.$http.post('{!! yzWebFullUrl('order.vue-operation.close') !!}',json).then(function (response) {
                    if (response.data.result) {
            ...
            sureCloseOrder1() {
                ...
                console.log(json);
                let loading = this.$loading({target:document.querySelector("#close-order"),background: 'rgba(0, 0, 0, 0)'});
                this.$http.post('{!! yzWebFullUrl('order.vue-operation.manualRefund') !!}',json).then(function (response) {
                    if (response.data.result) {
                        this.$message({type: 'success',message: '关闭订单成功!'});
                    }
                    else{
