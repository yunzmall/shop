...
function setProvinceData(v1,v2,v3,v4){
    let _pro_id = $("#sel-provance").attr("value");
    var _html = "";
    _html += '<option value="0">请选择省份</option>';
    for (var i = 0; i < _provinceNetworkData.length; i++) {
        var _selected = '';
        if (v1 == _provinceNetworkData[i].id || _provinceNetworkData[i].id  == _pro_id ) {
...
function setCityData(v1,v2,v3,v4){
    let _city_id = $("#sel-city").attr("value");
    var _html = '<option value="0">请选择城市</option>';
    for (var i = 0; i < _cityNetworkData.length; i++) {
        var _selected = '';
        if (v2 == _cityNetworkData[i].id || _cityNetworkData[i].id  == _city_id ) {
            _selected = 'selected="selected"';
        }
...
function setDistrictData(v1,v2,v3,v4){
    let _area_id = $("#sel-area").attr("value");
    var _html = "";
    _html += '<option value="0">请选择区域</option>';
    for (var i = 0; i < _districtNetworkData.length; i++) {
        var _selected = '';
        if (v3 == _districtNetworkData[i].id || _districtNetworkData[i].id  == _area_id) {
            _selected = 'selected="selected"';
        }
...
function setStreetData(v1,v2,v3,v4){
    let _street_id = $("#sel-street").attr("value");
    var _html = "";
    _html += '<option value="0">请选择街道</option>';
    for (var i = 0; i < _streetNetworkData.length; i++) {
        var _selected = '';
        if (v4 == _streetNetworkData[i].id || _streetNetworkData[i].id  == _street_id ) {
            _selected = 'selected="selected"';
        }
