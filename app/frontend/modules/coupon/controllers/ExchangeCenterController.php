...
class ExchangeCenterController extends ApiController
{
    ...
    public function index()
    {
        ...
        foreach ($result as $key => $value){
            ...
            $goods =  Goods::uniacid()
                ->select('id','has_option')
                ->with([
                    'hasManyOptions'=>function($q){
                        return $q->select('id','goods_id','title','product_price','market_price','stock');
                    }
                ])
                ->whereIn('id',$goodsIds)
                ->get();
            foreach($goods as $k=>$v){
                if($v['has_option'] == 1){
                    $array['list'][$i]['belongs_to_coupon']['goods_ids'][$i] =$v;
                }
            }
