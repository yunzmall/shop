...
class PosterController extends ApiController
{
    ...
    private function exist_poster($imageUrl)
    {
        if (\YunShop::plugin()->get('new-poster')) {
            //如果会员第一次生成海报就写入记录表
            $posterModel = \Yunshop\NewPoster\models\Poster::uniacid()->select('id', 'is_open')->where(['center_show' => 1, 'poster_type' => $this->type])->first();
            if(\Yunshop\NewPoster\models\PosterRecord::where('url',$imageUrl)->get()->isEmpty() && !empty($imageUrl) && $posterModel){
                $posterRecord = new \Yunshop\NewPoster\models\PosterRecord();
                ...
                $posterRecord->poster_type = $this->type;
                $posterRecord->save();
            }
        } elseif (\YunShop::plugin()->get('poster')) {
            //如果会员第一次生成海报就写入记录表
    ...
    private function getPoster()
    {
        if (\YunShop::plugin()->get('new-poster')) {
            ...
            if (($posterModel && $posterModel->is_open) || ($posterModel && !$posterModel->is_open && Member::current()->yzMember->is_agent)) {
                $file_path = (new \Yunshop\NewPoster\services\CreatePosterService(\YunShop::app()->getMemberId(), $posterModel->id, $this->type))->getMemberPosterPath();
                if (!$file_path) {
                    return '';
                }
