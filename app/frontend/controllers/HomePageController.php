...
class HomePageController extends ApiController
{
    ...
    public function index($request, $integrated = null)
    {
        ...
        try {
            ...
            {   
                // 需要登陆的组件 签到，门店，优惠券，大转盘，表单
                $LoginComponent = ['stores','signin','lottery','form','coupons','tabcontrol'];
                //系统信息
                ...
                { //如果是请求首页的数据, 提供默认值
                    ...
                    //登陆跳转
                    if (!$member_id && !$basic_info) 
                    {
                        if ($decorate['member_level'] !== '') 
                        {
                            return $this->jumpUrl($type,1);
                        }

                        foreach ($LoginComponent as $key => $value) 
                        {
                            if (in_array($value, $decorate['temps'])) 
                            {
                                return $this->jumpUrl($type,1);
                            }
                        }
                        ...
                        {
                            if ($value['component_key'] === 'U_goods' && array_intersect([4,5,6,8],$value['remote_data']['content_list'])) 
                            {
                                return $this->jumpUrl($type,1);
                            }
                        }
            ...
            elseif (empty($pageId)) 
            {  //如果是请求首页的数据, 但是没有安装"装修插件"或者"装修插件"没有开启, 则提供默认值
                $result['default'] = self::defaultDesign();
                $result['item']['menus'] = self::defaultMenu($i, $mid, $type);
                $result['item']['menustyle'] = self::defaultMenuStyle();
                $result['item']['data'] = ''; //前端需要该字段
...
//        }
    ...
    public function getCaptcha($request, $integrated = null)
    {
        //增加验证码功能
        $status = Setting::get('shop.sms.status');
            if ($status == 1) {
