...
class UploadController extends BaseController
{
    ...
    public function saveData($type, $fullname, $originname, $ext, $filename, $module_upload_dir, $pathname, $option)
    {
       ...
       // dd($type, yz_tomedia($pathname), $ext);
        //内容审核
        if(app('plugins')->isEnabled('upload-verification')) {
            if ($type == 'image') {
                    //图片审核
                    if (in_array(strtolower($ext), ['png','jpg','jpeg','bmp','gif','webp','tiff'])) {
                        $uploadReuslt = do_upload_verificaton(yz_tomedia($pathname), 'img');
                ...
                //视频审核
                $uploadReuslt = do_upload_verificaton(yz_tomedia($pathname), 'video');
                if (0 === $uploadReuslt[0]['status']) {
                    return $this->errorJson($uploadReuslt[0]['msg']);
                }
            }
