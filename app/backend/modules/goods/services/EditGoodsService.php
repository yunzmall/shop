...
class EditGoodsService
{
    ...
    public function edit()
    {
        ...
        if ($goods_data) {
            ...
            if (empty($this->request->widgets['sale']['point_deduct_type']) && !empty($this->request->widgets['sale']['max_point_deduct']) && !empty($goods_data['price'])
            ...
            $save_data = array_except(
                $goods_data,
                ['category', 'withhold_stock', 'video_image', 'goods_video', 'category_to_option']
            );
    ...
    public function oldedit()
    {
        ...
        foreach ($this->goods_model->hasManySpecs as &$spec) {
            $spec['items'] = GoodsSpecItem::where('specid', $spec['id'])->orderBy('display_order', 'asc')->get(
            )->toArray();
        }
