    ...
    public function index()
    {
        ...
        if ($searchTime) {
            $orderAndUnwithdraw = DB::table('yz_store as s')
                ...
                ->selectRaw($prefix.'s.id,max(store_name) as name, max(thumb) as thumb_url, sum(price) as price')
                ->groupBy('s.id')
                ->get();
            ...
            $withdraws = DB::table('yz_store as s')
                ...
                ->where('s.uniacid', $uniacid)
                ->where('sw.incometable_type','Yunshop\StoreCashier\common\models\StoreOrder')
                ->selectRaw($prefix.'s.id,sum(if('.$prefix.'sw.status=0, '.$prefix.'sw.amount,0)) as un_withdraw, sum(if('.$prefix.'sw.status=1 and '.$prefix.'sw.pay_status!=2 ,'.$prefix.'sw.amount,0)) as withdrawing, sum(if('.$prefix.'sw.pay_status=2 ,'.$prefix.'sw.amount,0)) as withdraw')
                ->groupBy('s.id')
                ->get();
        } else {
            $orderAndUnwithdraw = DB::table('yz_store as s')
