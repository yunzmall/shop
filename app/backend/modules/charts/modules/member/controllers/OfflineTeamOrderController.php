...
class OfflineTeamOrderController extends BaseController
{
    ...
    public function index(){
        ...
        $list = MemberLowerGroupOrder::getMember($search)->orderBy('amount', 'desc')->paginate($pageSize);
...
//        }
//
//        $sql = '(select child_id, member_id,
//        count(child_id) as team_next_count,
//        count(( select 1 from '. DB::getTablePrefix() .'yz_order where '. DB::getTablePrefix() .'yz_order.uid = '. DB::getTablePrefix() .'yz_member_children.child_id and '. DB::getTablePrefix() .'yz_order.`status` >= 1 limit 1 )) as pay_count,
//        sum(( select sum(goods_total) from '. DB::getTablePrefix() .'yz_order where '. DB::getTablePrefix() .'yz_member_children.child_id = '. DB::getTablePrefix() .'yz_order.uid and '. DB::getTablePrefix() .'yz_order.`status` >= 1 )) as order_goods_total,
//        sum(( select sum(price) from '. DB::getTablePrefix() .'yz_order where '. DB::getTablePrefix() .'yz_member_children.child_id = '. DB::getTablePrefix() .'yz_order.uid and '. DB::getTablePrefix() .'yz_order.`status` >= 1 )) as order_price,'
//            . DB::getTablePrefix() .'mc_members.uid, avatar, nickname from '. DB::getTablePrefix() .'yz_member_children inner join '. DB::getTablePrefix() .'mc_members on '. DB::getTablePrefix() .'yz_member_children.member_id = '. DB::getTablePrefix() .'mc_members.uid where '. DB::getTablePrefix() .'yz_member_children.uniacid = ' . \YunShop::app()->uniacid . ' group by '. DB::getTablePrefix() .'yz_member_children.member_id order by `team_next_count` desc) as cc';
//
