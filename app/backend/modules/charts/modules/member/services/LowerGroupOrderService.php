...
class LowerGroupOrderService
{
    ...
    public function memberOrder(){
        ...
        foreach ($uniAccount as $u) {
            ...
            $insert_data = [];
            //查询归类所有订单开始
            $order_data = DB::table('yz_order')->select('uid', DB::raw('SUM(price) as amount'), DB::raw('COUNT(id) as count') , DB::raw('SUM(goods_total) as goods_count'))
                ->where('status','>',0)->where('uniacid', \YunShop::app()->uniacid)->groupBy('uid')->get()->toArray();
...
//            $isset_uid = member::uniacid()->pluck('uid')->toArray();
            ...
            MemberLowerGroupOrder::uniacid()->delete();
