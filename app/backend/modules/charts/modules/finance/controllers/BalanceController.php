...
//        dd($balanceTime);
        ...
        foreach ($this->time as $key => $time) {
            $allBalanceData[$key] = Balance::uniacid()
                ...
                ->selectRaw('sum(if(service_type=1 && operator=0,change_money,0)) as recharge, sum(if(service_type=1 && operator=-2,change_money,0)) as memberRecharge')
...
//                ->where('yz_balance.created_at','>=',strtotime("$time"))
                ->where('yz_balance.created_at','<=', strtotime("$time 23:59:59"))
